...
Conventional BiLSTM based methods use word representations or sentence representations independently to improve the performance.
Alternatively, the proposed method takes account of both representations to extract the latent semantics, which can not be captured by previous methods.
The experimental results show that the proposed method can significantly improve the performances.
We get an absolute improvement of 3.2 points in F1 score compared with BiLSTM based conventional method using linguistic features.
