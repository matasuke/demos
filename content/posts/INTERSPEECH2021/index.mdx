...
This paper propose a novel method for phrase break prediction in Japanese Text-to-speech (TTS) System.
The proposed method uses implicit features extracted from a pre-trained large language model, a.k.a BERT, and explicit features extracted from BiLSTM with linguistic features, such as part of speech tags.
The proposed method is compared both in an objective evaluation based on F-scores but also in a subjective evaluation based on MOS test and AB preference test. We get an absolute improvement of 3.2 points in F1 score compared with BiLSTM based conventional method using linguistic features explicitly.
Moreover, Perceptual listening test results in terms of the naturalness of prosody verify that TTS system applying our proposed method achieves 4.39 mean opinion score, which is comparative to the synthesized speech samples using reference text.

![PBP architecture](./pbp_architecture.jpg)
